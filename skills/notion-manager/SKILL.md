...
- 用户只是随口提到 Notion，没有明确的操作意图（如"我的 Notion 好乱"）
- 用户只是在问关于 Notion 的问题，不需要实际操作工作区
- 用户只是在问关于本 skill 的问题
- 输入过于模糊，无法判断意图是查询、归类还是写入——此时先用意图澄清流程确认，而不是直接触发
...
1. **写入操作执行前必须确认**：告知用户操作类型、目标页面、内容摘要，等用户确认后才执行——没有例外
2. **Token 不明文保留**：用户在对话里提供的 token 用完即走，不存储，不重复引用
3. **不支持删除**：任何删除请求一律引导用户去 Notion 界面操作，AI 不执行
4. **不支持 Database 类型页面**：遇到 Database 时明确告知用户，不静默跳过
5. **快照过期时必须提示**：如果快照状态不确定，不能用旧快照做写入决策，必须先告知用户
...
- 没拿到 token 就尝试执行任何 API 操作，静默失败而不提示用户
- 把模糊请求当成明确写入指令，跳过意图澄清直接执行
- 写入操作未经确认直接执行
- 快照可能过期，但未提示用户，直接用旧快照做归类或写入判断
- 把"帮我记一下"直接理解为写入，而不先走 notes-protocol 流程确认是否需要生成笔记
- 权限报错时静默降级，而不告知用户检查 Integration 共享设置