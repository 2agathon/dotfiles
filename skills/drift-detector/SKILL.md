...
- 没有基线就给出强漂移判断，把"感觉变了"直接写成漂移结论
- 把普通 diff（措辞变化、格式调整、局部补充）当作漂移报告输出
- 把健康演化（系统主动承认的方向调整、长出新对象）判成系统变质
- D3 身份漂移候选被当作普通检测结论输出，而不是明确标注为"候选，需人工审视"
- 把"味道变了"直接写成漂移证据，而不去找可引用的文本依据
- 过度敏感，所有细微变化都触发漂移报告，导致系统噪音过高
